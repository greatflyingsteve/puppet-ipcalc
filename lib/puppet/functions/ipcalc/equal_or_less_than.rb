...
Puppet::Functions.create_function(:"ipcalc::equal_or_less_than") do
  ...
  # (or possibly less than) the right.
  #
  ...
  # @return [Boolean]
  #   `true` if the left operand is equal to (or less than) the right, or `false` if it's greater
  #   than the right.
  # @example Use with dotted function notation
  #   $left_ip.equal_or_less_than($right_ip) ? {
  #     true    => { 'We did it, go team, left operand is equal or smaller' },
  ...
  def normalize_netaddrs(left, right)
    ...
    equal_or_less_than(left.split('/')[0], right.split('/')[0], left_netaddr, right_netaddr)
  end

  def equal_or_less_than(left, right, left_netaddr = IPAddr.new(left), right_netaddr = IPAddr.new(right))
