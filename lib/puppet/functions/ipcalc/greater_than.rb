...
Puppet::Functions.create_function(:"ipcalc::greater_than") do
  # Compare two IP addresses and return a Boolean indicating whether the left operand is greater
  # than the right.
  #
  ...
  # @return [Boolean]
  #   `true` if the left operand is greater than the right, or `false` otherwise.
  # @example Use with dotted function notation
  #   $left_ip.ipcalc::greater_than($right_ip) ? {
  #     true    => { 'We did it, go team, left operand is bigger' },
  ...
  def normalize_netaddrs(left, right)
    ...
    greater_than(left.split('/')[0], right.split('/')[0], left_netaddr, right_netaddr)
  end

  def greater_than(left, right, left_netaddr = IPAddr.new(left), right_netaddr = IPAddr.new(right))
