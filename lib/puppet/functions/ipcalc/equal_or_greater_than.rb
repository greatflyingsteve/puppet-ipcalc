...
Puppet::Functions.create_function(:"ipcalc::equal_or_greater_than") do
  ...
  # (or possibly greater than) the right.
  #
  ...
  # @return [Boolean]
  #   `true` if the left operand is equal to (or greater than) the right, or `false` if it's less
  #   than the right.
  # @example Use with dotted function notation
  #   $left_ip.equal_or_greater_than($right_ip) ? {
  #     true    => { 'We did it, go team, left operand is equal or bigger' },
  ...
  def normalize_netaddrs(left, right)
    ...
    equal_or_greater_than(left.split('/')[0], right.split('/')[0], left_netaddr, right_netaddr)
  end

  def equal_or_greater_than(left, right, left_netaddr = IPAddr.new(left), right_netaddr = IPAddr.new(right))
